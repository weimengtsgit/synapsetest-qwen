...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if podman exec $CONTAINER_NAME mysqladmin ping -h localhost -uroot -p$MYSQL_ROOT_PASSWORD --silent 2>/dev/null; then
        echo ""
        break
    fi
    echo -n "."
...
if podman ps | grep -q "$CONTAINER_NAME"; then
    ...
    echo ""
    echo "🔧 常用命令:"
    echo "  连接数据库: podman exec -it $CONTAINER_NAME mysql -uroot -p$MYSQL_ROOT_PASSWORD"
    echo "  进入容器: podman exec -it $CONTAINER_NAME bash"
    echo "  查看日志: podman logs -f $CONTAINER_NAME"
    echo "  停止服务: podman stop $CONTAINER_NAME"
    echo "  启动服务: podman start $CONTAINER_NAME"
    echo "  删除容器: podman rm -f $CONTAINER_NAME"
    echo ""
    echo "📝 快速连接命令:"
    ...
    # 显示数据库列表
    echo "📚 当前数据库列表:"
    podman exec $CONTAINER_NAME mysql -uroot -p$MYSQL_ROOT_PASSWORD -e "SHOW DATABASES;" 2>/dev/null | tail -n +2
